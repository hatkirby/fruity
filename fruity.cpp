...
int main(int argc, char** argv)
{
  ...
  verbly::noun fruit = database.nouns().with_wnid(113134947).run().front(); // fruit
  ...
  for (;;)
  {
    ...
    auto n1 = database.nouns().full_hyponym_of(fruit).random().limit(1).run();
    auto n1w = n1.front();
    ...
    if (!similar.empty())
    {
      ...
      auto different = database.nouns().full_hyponym_of(fruit).except(n1w).is_not_proper().with_complexity(1).random().limit(1).run();
